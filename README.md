...
<img src="Assets/MouseTalk.png" width="96" alt="妙语：结合鼠标、老鼠头像和声波的标志">

**把无线鼠标变成语音输入的遥控器。随口说，随手发。**

**[前往 Try AI Lab 妙语主页，查看介绍与下载 MouseTalk →](https://www.tryailab.com/projects/mousetalk)**

已经在用豆包输入法或其他语音输入软件，却还要伸手按键盘才能开始说话？妙语让你用鼠标侧键触发同一个快捷键，再用另一个按钮按回车发送，方便你靠在沙发上与 Agent 交流。

妙语负责“替你按键”，语音识别仍由你已有的软件完成。它不会录音，也不需要登录或联网。
...
**快捷键需兼容：**妙语目前可以模拟单按 Fn、左／右 Control、Option、Command，或同时按其中两个键。请在语音软件里选择对应的单按快捷键；暂不支持双击、长按说话，或带字母／空格的组合。不是所有语音软件都支持这些设置。
...
1. **权限**：点击“去设置”，开启妙语的“输入监控”和“辅助功能”。回到妙语会自动刷新；若仍未生效，点击“重新打开”。
2. **语音快捷键**：选成和豆包等输入法里一样的按键即可。比如豆包设为“右 Option”，妙语也选“右 Option”，第二个键选“无”。**无需选择软件。**
...
“高级”中可以把双击右键设为复制或粘贴。为保留正常的单击右键，开启后妙语会等待一个系统双击间隔再发送单击，因此右键菜单会略有延迟；不需要时保持关闭。

设置自动保存。绑定后自动启用，替换按钮原来的动作；关闭右上角“启用”即可恢复。关闭窗口后妙语仍在菜单栏运行，可从菜单栏打开设置或退出。

还没有语音输入法，可以从应用里的链接前往[豆包输入法官方下载页](https://ime.doubao.com/pc)。

### 权限开关已经开启，却显示“未生效”

系统开关与当前应用的实际授权有时不一致，尤其是重新构建或更换签名之后。妙语显示系统返回的实际权限，不会仅凭开关外观显示已授权。

- 先点击“重新打开”，排除当前进程尚未刷新权限的情况。
- 仍无效时，点击对应权限旁的 **？ → 清除旧授权并前往设置**。它只重置妙语的这一项权限，然后打开当前应用所在位置和系统设置。
- 在系统设置中用 **＋** 添加当前的 **MouseTalk.app** 并开启，再重新打开妙语。这个操作需要你亲自在系统设置中授权。
...
| 键盘能用，鼠标不行 | 妙语的两项权限是否开启、按钮是否绑定、鼠标控制是否启用；两边的按键及左右侧是否一致 |
| 两边设置一致，仍无法触发 | 展开“高级”，尝试兼容模式；部分软件可能不接受模拟按键 |
| 绑定时识别不到鼠标按钮 | 妙语只识别标准鼠标按钮；检查 Logi Options+ 等驱动是否把它改成了手势或其他按键 |
...
妙语监听标准鼠标按钮事件，通过 Core Graphics 发送模拟按键，不生成真实硬件 HID 报告。已绑定额外按钮的按下事件会被拦截，抬起事件会放行，避免系统留下鼠标按下的状态。启用双击右键动作后，单击右键会被短暂保留并在超时后重放。
...
妙语的按键功能不联网、不收集分析数据、不录音，也不读取你输入的文字。设置页和菜单栏的 GitHub／提供反馈按钮只在点击时打开外部浏览器，不自动发送配置或诊断信息。语音软件如何处理录音取决于那个软件。更多说明见 [SECURITY.md](SECURITY.md)。
